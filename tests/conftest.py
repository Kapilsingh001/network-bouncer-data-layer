...
def dirty_records() -> pd.DataFrame:
    ...
    #   5 null proto | 6 duplicate-of-0
    # NOTE: protocols are NOT whitelisted — only blank/null protocols are dropped,
    # so the proto defect here is a missing value, not an "unknown" token.
    return pd.DataFrame(
        {
            ...
            "proto": ["tcp", "tcp", "udp", "udp", "tcp", None, "tcp"],
